...
function SelectionComp(props: SelectionProps){
    const {options, placeholder, onChange, initialValue, useFor} = props
    ...
    if(useFor == 'select-period-extract'){
        return(
                <>
    ...
    } else if(useFor == 'order-moviments'){
        return(
            <>
            <div className="relative" ref={wrapperRef}>
                <button type="button"
                        className={`flex w-full border px-2 py-1 items-center
                                    justify-between border-white/30 rounded-md h-7
                                    text-xs font-extralight text-white/50
                                    ...
                                    <li key={index}>
                                        <button type="button"
                                                ...
                                                className={`w-full text-left px-3 py-2 text-sm
                                                            transition-colors
                                                            ${selectedOption?.value === option.value ? 
                                                                'text-white font-light' : 'text-white/80 hover:bg-white/5 hover:text-white'
                                                            }`} >
                                                {option.label}
                                        </button>
                                        {index < options.length - 1 && (
                                            <div className="mx-2 h-px bg-white/50"></div>
                                        )}
                                    </li>
                                ))}
                            </ul>
    ...
    } else if(useFor == 'select-type-payment'){
        return(
                <>
                    ...
                    {isOpen && (
                        <div className="absolute right-2.5 z-50 mt-3.5  w-full bg-[#18181b]/50 backdrop-blur-sm rounded-2xl border border-white/50  overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                            <ul className="max-h-64 overflow-y-auto py-1 ">
                                {options.map((option, index) => (
                                    <li key={index}>
                                        <button
    ...
    } else if(useFor == 'select-category'){
        return(
                <>
                    <div className="relative w-full" ref={wrapperRef}>
                        <button type="button"
                                className={`
                                            flex items-center justify-between w-full h-4 px-2.5 py-3
                                            border border-white/25 rounded-lg 
                                            text-sm font-normal focus:outline-none transition-all duration-200
                                            ${isOpen ? 'ring-1 ring-purple-500' : ''}
                                        `}
                                onClick={()=>setIsOpen(!isOpen)} >
                            <span className={`${selectedOption ? "text-white" : "text-white/50"}`}>
